...
CSV_PATH = BASE_DIR / "skater_timeonice.csv"
...
desired_order = [
    ...
    "gamesPlayed", "timeOnIce", "evTimeOnIce", "ppTimeOnIce",
    "shTimeOnIce", "timeOnIcePerGame", "evTimeOnIcePerGame",
    "ppTimeOnIcePerGame", "shTimeOnIcePerGame", "otTimeOnIce",
    "otTimeOnIcePerOtGame", "shifts", "timeOnIcePerShift",
    "shiftsPerGame", "lastName"
]
...
st.subheader("Skater Time On Ice")
...
with filter_col5:
    if has_player:
        search = st.text_input("Search player", key="toi_search")
        if search:
            filtered_df = filtered_df[
...
if "toi_page_num" not in st.session_state:
    st.session_state.toi_page_num = 1

if "toi_last_filter_count" not in st.session_state:
    st.session_state.toi_last_filter_count = total_rows

if st.session_state.toi_last_filter_count != total_rows:
    st.session_state.toi_page_num = 1
    st.session_state.toi_last_filter_count = total_rows

if st.session_state.toi_page_num > total_pages:
    st.session_state.toi_page_num = total_pages
if st.session_state.toi_page_num < 1:
    st.session_state.toi_page_num = 1

start_idx = (st.session_state.toi_page_num - 1) * page_size
...
with col1:
    if st.button("◀ Back", disabled=st.session_state.toi_page_num <= 1, key="toi_back"):
        st.session_state.toi_page_num -= 1
        st.rerun()

with col2:
    st.markdown(
        f"<div style='text-align:center; padding-top:0.5rem;'>Page {st.session_state.toi_page_num} of {total_pages}</div>",
        unsafe_allow_html=True
    )

with col3:
    if st.button("Next ▶", disabled=st.session_state.toi_page_num >= total_pages, key="toi_next"):
        st.session_state.toi_page_num += 1
        st.rerun()
