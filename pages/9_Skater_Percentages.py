...
CSV_PATH = BASE_DIR / "skater_percentages.csv"
...
desired_order = [
    ...
    "gamesPlayed", "timeOnIcePerGame5v5", "satPercentage",
    "satPercentageAhead", "satPercentageTied", "satPercentageBehind",
    "satPercentageClose", "satRelative", "usatPercentage",
    "usatPercentageAhead", "usatPercentageTied", "usatPercentageBehind",
    "usatPrecentageClose", "usatRelative", "zoneStartPct5v5",
    "shootingPct5v5", "skaterSavePct5v5", "skaterShootingPlusSavePct5v5",
    "lastName"
]
...
st.subheader("Skater Percentages")
...
with filter_col5:
    if has_player:
        search = st.text_input("Search player", key="pct_search")
        if search:
            filtered_df = filtered_df[
...
if "pct_page_num" not in st.session_state:
    st.session_state.pct_page_num = 1

if "pct_last_filter_count" not in st.session_state:
    st.session_state.pct_last_filter_count = total_rows

if st.session_state.pct_last_filter_count != total_rows:
    st.session_state.pct_page_num = 1
    st.session_state.pct_last_filter_count = total_rows

if st.session_state.pct_page_num > total_pages:
    st.session_state.pct_page_num = total_pages
if st.session_state.pct_page_num < 1:
    st.session_state.pct_page_num = 1

start_idx = (st.session_state.pct_page_num - 1) * page_size
...
with col1:
    if st.button("◀ Back", disabled=st.session_state.pct_page_num <= 1, key="pct_back"):
        st.session_state.pct_page_num -= 1
        st.rerun()

with col2:
    st.markdown(
        f"<div style='text-align:center; padding-top:0.5rem;'>Page {st.session_state.pct_page_num} of {total_pages}</div>",
        unsafe_allow_html=True
    )

with col3:
    if st.button("Next ▶", disabled=st.session_state.pct_page_num >= total_pages, key="pct_next"):
        st.session_state.pct_page_num += 1
        st.rerun()
